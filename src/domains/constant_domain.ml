...
module Constants = (struct
  ...
  let mul a b = if ((a = Cst Z.zero) || (b = Cst Z.zero)) then (const Z.zero)  else (lift2 Z.mul a b)
  ...
  | AST_DIVIDE ->
      (* r=x/y => (x=r*y or 0) and (y=r*x or 0)  *)
      let contains_zero o = subset (const Z.zero) o in
      (if contains_zero y || contains_zero r then (const Z.zero) else meet x (mul r y)),
      (if contains_zero x || contains_zero r then (const Z.zero) else meet y (mul r x))
