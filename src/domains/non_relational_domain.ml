...
module NonRelational(V : VALUE_DOMAIN) = (struct
  module V = struct
    ...
    let unary v op =
      let rep = unary v op in
      let () = debug_int_unary_op (string_of_int_unary_op op) v rep in
      rep

    let binary v1 v2 op =
      let rep = binary v1 v2 op in
      let () = debug_int_binary_int_op (string_of_int_binary_op op) v1 v2 rep in
      rep

    let const z =
      let rep = const z in
      if !non_relational_debug then Format.printf "%sconst %a ~> %a@." debugstr Z.pp_print z print rep;
      rep

    let rand z1 z2 =
      let rep = rand z1 z2 in
      if !non_relational_debug then Format.printf "%srand %a %a ~> %a@." debugstr Z.pp_print z1 Z.pp_print z2 print rep;
      rep

    let join v1 v2 =
      let rep = join v1 v2 in
      let () = debug_int_binary_int_op "⊔" v1 v2 rep in
      rep

    let meet v1 v2 =
      let rep = meet v1 v2 in
      let () = debug_int_binary_int_op "⊓" v1 v2 rep in
      rep

    let widen v1 v2 =
      let rep = widen v1 v2 in
      let () = debug_int_binary_int_op "▿" v1 v2 rep in
      rep

    let subset v1 v2 =
      let rep = subset v1 v2 in
      if !non_relational_debug then Format.printf "%s%a ⊏ %a ~> %b@." debugstr V.print v1 V.print v2 rep ;
      rep

    let is_bottom v =
      let rep = is_bottom v in
      if !non_relational_debug then Format.printf "%sis_bottom %a ~> %b@." debugstr V.print v rep ;
      rep

    let compare v1 v2 cmp_op =
      let rep1, rep2 = compare v1 v2 cmp_op in
      let () = debug_int_binary_int_int_op (string_of_compare_op cmp_op) v1 v2 rep1 rep2 in
      rep1, rep2

    let bwd_unary v op v' =
      let rep = bwd_unary v op v' in
      if !non_relational_debug then Format.printf "%s%s %a (bwd from %a) ~> %a@."
          debugstr
    ...
    let bwd_binary v1 v2 op v' =
      let rep1, rep2 = bwd_binary v1 v2 op v' in
      if !non_relational_debug then Format.printf "%s%a %s %a (bwd from %a) ~> %a, %a@."
          debugstr
          V.print v1
