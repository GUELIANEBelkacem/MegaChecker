    ...
    | AST_while (e,s) ->
        ...
        let rec fix (f:t -> t) (x:t) (n:int) : t = 
          ...
          if D.subset fx x then fx
            ...
            if n< !delay then fix f fx (n+1)
            else fix f (D.widen x fx ) n
        in
        *)
        let rec fix (f:t -> t) (x:t) (d:int): t = 
          ...
          if D.subset fx x then fx
            ...
            if d=0 then fix f (D.widen x fx) !delay
            else fix f (D.join x fx) (d-1)
          )
          in
         ...
         *)        
         let rec loop_unroll (n:int) (f:t -> t) (x:t): t =
          if n=0 then x
          else (
            if D.is_bottom (filter x e true) then x
            else loop_unroll (n-1) f (f x)
          )
        in let no_join x = eval_stat (filter x e true) s
        in if D.is_bottom (filter a e true) then a
            else let a' = loop_unroll !unroll no_join a in

        let f x = D.join a' (eval_stat (filter x e true) s) in
        (* compute fixpoint from the initial state (i.e., a loop invariant) *)
        let inv = D.join (fix f a' !delay) (filter a e false) in
        (* and then filter by exit condition *)
        filter inv e false

    | AST_assert e ->
       let f = filter a e false in
       if (D.is_bottom f) then (filter a e true)
       else (error ext "assertion failure"; (filter a e true))
